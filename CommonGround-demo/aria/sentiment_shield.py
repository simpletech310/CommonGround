...
Monitors parent-to-parent communication and flags potentially hostile messages.
Offers gentle suggestions while allowing parents to make their own choices.
"""
...
class ToxicityLevel(Enum):
    ...
    NONE = "none"           # Message is fine
    LOW = "low"             # Slightly negative tone
    MEDIUM = "medium"       # Could be taken poorly
    HIGH = "high"           # Clearly hostile/toxic
    SEVERE = "severe"       # Abusive/threatening


class ToxicityCategory(Enum):
    """Categories of toxic patterns"""
    PROFANITY = "profanity"
    INSULT = "insult"
    HOSTILITY = "hostility"
    SARCASM = "sarcasm"
...
class SentimentAnalysis:
    ...
    triggers: List[str]  # Specific words/phrases that triggered
    explanation: str
...
class SentimentShield:
    """
    ARIA's Sentiment Shield
    Analyzes messages for toxicity and offers gentle suggestions.
    """
    
    # Profanity and hostile language patterns
    PROFANITY_PATTERNS = [
    ...
    # Suggested replacements for common hostile phrases
    SUGGESTIONS = {
    ...
    def __init__(self):
        """Initialize the sentiment shield"""
        self.compiled_patterns = self._compile_patterns()
    
    def _compile_patterns(self) -> Dict[ToxicityCategory, List[re.Pattern]]:
        """Pre-compile regex patterns for efficiency"""
        return {
            ToxicityCategory.PROFANITY: [re.compile(p, re.IGNORECASE) for p in self.PROFANITY_PATTERNS],
            ToxicityCategory.INSULT: [re.compile(p, re.IGNORECASE) for p in self.INSULT_PATTERNS],
            ToxicityCategory.HOSTILITY: [re.compile(p, re.IGNORECASE) for p in self.HOSTILITY_PATTERNS],
            ToxicityCategory.DISMISSIVE: [re.compile(p, re.IGNORECASE) for p in self.DISMISSIVE_PATTERNS],
            ToxicityCategory.PASSIVE_AGGRESSIVE: [re.compile(p, re.IGNORECASE) for p in self.PASSIVE_AGGRESSIVE_PATTERNS],
            ToxicityCategory.BLAME: [re.compile(p, re.IGNORECASE) for p in self.BLAME_PATTERNS],
            ToxicityCategory.THREATENING: [re.compile(p, re.IGNORECASE) for p in self.THREATENING_PATTERNS],
        }
    
    def analyze(self, message: str, context: List[str] = None) -> SentimentAnalysis:
        """
        ...
        if len(words) > 0 and caps_words / len(words) > 0.5:
            ...
            triggers.append("EXCESSIVE CAPS (shouting)")
        ...
        explanation = self._generate_explanation(categories, triggers)
    ...
    def _calculate_score(self, categories: List[ToxicityCategory], triggers: List[str]) -> float:
        """Calculate a toxicity score from 0.0 to 1.0"""
        if not categories:
            return 0.0
    ...
    def _generate_explanation(self, categories: List[ToxicityCategory], triggers: List[str]) -> str:
    ...
    def _generate_suggestion(self, message: str, categories: List[ToxicityCategory]) -> str:
        ...
        # Remove excess profanity that wasn't caught
        ...
        # If suggestion is too empty or same as original, provide a template
    ...
    def get_aria_message(self, analysis: SentimentAnalysis) -> str:
        """Get ARIA's intervention message based on analysis"""
        ...
        header = level_messages.get(analysis.toxicity_level, "💭 **A gentle note...**")
        
        # Build the message
        lines = [
            header,
            "",
            f"I noticed this message {analysis.explanation.lower()}",
            "",
            "Remember, your little one(s) are counting on both of you to communicate kindly. 💙",
            "",
            "**Your original message:**",
            f"> {analysis.original_message}",
            "",
        ]
        
        if analysis.suggestion:
            lines.extend([
                "**A gentler way to say this:**",
        ...
        lines.extend([
            ...
            "  [1] ✏️  Use ARIA's suggestion",
            "  [2] 📝 Edit my message",
            "  [3] 📤 Send original anyway",
            "  [4] 🗑️  Cancel message",
        ])
...
class ConversationContext:
    """Tracks conversation context for better analysis"""
    
    def __init__(self, max_history: int = 10):
        self.messages: List[Dict[str, Any]] = []
        self.max_history = max_history
    
    def add_message(self, sender: str, message: str, was_flagged: bool = False):
    ...
    def get_recent_messages(self, count: int = 5) -> List[str]:
        ...
        return [m["message"] for m in self.messages[-count:]]
    
    def get_tension_level(self) -> float:
        """Estimate current conversation tension based on recent flags"""
        if not self.messages:
            return 0.0
        
        recent = self.messages[-5:]
        flagged_count = sum(1 for m in recent if m["was_flagged"])
        return flagged_count / len(recent)
